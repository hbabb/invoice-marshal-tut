...
export async function GET(
  ...
  pdf.text(formatCurrency({ amount: data.total, currency: (data.currency as "USD") || "EUR" }), 160, 110);

  // Total Section
  pdf.line(20, 115, 190, 115);
  pdf.setFont("helvetica", "bold");
  pdf.text(`Total (${data.currency})`, 130, 130);
  pdf.text(formatCurrency({ amount: data.total, currency: (data.currency as "USD") || "EUR" }), 160, 130);
